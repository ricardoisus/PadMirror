...
## Build from source
For a drag-and-drop Mac installer, see [installation instructions](docs/INSTALL.md) and [USB + AirPlay download](https://github.com/ricardoisus/PadMirror/releases/tag/v0.1.0-airplay-preview.1). The complete installer includes USB + AirPlay and its runtime dependencies, without Apple notarization. Check the macOS minimum and architecture on the release; the local complete build requires macOS 26.6.2+ on Apple Silicon. The older USB-only preview remains available for macOS 14+.
