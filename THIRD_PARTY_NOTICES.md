...
GStreamer and GLib (LGPL-2.1 family), libplist (LGPL), OpenSSL (Apache-2.0), FFmpeg/libav and its codec dependencies (including GPL components such as x264/x265), llhttp (MIT), and their transitive dependencies retain their own licenses. Exact versions, available license texts and Homebrew build-source references are copied to Contents/Resources/ThirdPartyNotices and airplay-build.json in wireless builds. Source URLs/build recipes for dependencies are in their package metadata; publish corresponding source materials with any public binary release, including engine pins and local patches. No public release has been issued in this milestone.
